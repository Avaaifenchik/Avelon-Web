    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <p>1.1. Настоящий документ — публичная оферта Сервиса SaintProject на оказание услуг хостинга и аренды вычислительных мощностей. Акцепт оферты означает согласие с ее условиями.</p>
          <p>1.2. Оплата услуг любым способом или начало использования Сервиса означает полное и безоговорочное согласие с условиями настоящей оферты.</p>
          <h3 className="font-bold">2. ПРЕДМЕТ ДОГОВОРА</h3>
          <p>2.1. Сервис предоставляет Пользователю услуги по аренде вычислительных мощностей и виртуальных игровых серверов на условиях «как есть» (as is).</p>
          <h3 className="font-bold">3. ОПЛАТА УСЛУГ</h3>
          <p>3.1. Оплата производится в рублях или криптовалюте через платежные агрегаторы на сайте. Все комиссии платежных систем и сетей оплачиваются Пользователем.</p>
          <p>3.2. Средства зачисляются на внутренний баланс Пользователя после подтверждения транзакции и используются для авансовой оплаты услуг.</p>
          <h3 className="font-bold">4. ВОЗВРАТ СРЕДСТВ</h3>
          <p>4.1. Возврат неиспользованного остатка средств с баланса возможен в течение 14 дней с момента пополнения за вычетом фактически понесенных расходов Сервиса (включая комиссии платежных систем). Средства за уже активированные и предоставленные в аренду серверы/мощности возврату не подлежат.</p>
        </div>
      </div>
    )
  },
  terms: {
    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          <h3 className="font-bold">1. ОБЩИЕ ПОЛОЖЕНИЯ</h3>
          <p>1.1. Оплата или начало использования Услуг SaintProject означает полное и безоговорочное согласие Пользователя с условиями настоящего Соглашения (акцепт).</p>
          <h3 className="font-bold">2. ПРЕДМЕТ СОГЛАШЕНИЯ</h3>
          <p>2.1. Сервис предоставляет Пользователю вычислительные мощности (игровых серверов) на условиях подписки «как есть» (as is).</p>
          <h3 className="font-bold">3. ЗАПРЕЩЁННАЯ ДЕЯТЕЛЬНОСТЬ</h3>
          <p>3.1. Запрещены: DDoS-атаки, ботнеты, спам, фишинг, размещение нелегального контента, сканирование сетей, распространение вредоносного ПО, майнинг и действия, создающие аномальную нагрузку на инфраструктуру. При выявления нарушений Сервис вправе мгновенно заблокировать или удалить услугу без предварительного уведомления и без возврата денежных средств.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <p>1.1. Сервис SaintProject обрабатывает данные Пользователя (email, технические данные: IP-адрес, cookie, данные об оплате) исключительно в целях идентификации Пользователя, предоставления доступа к Услугам и обработки платежей.</p>
          <h3 className="font-bold">2. ЗАЩИТА ДАННЫХ</h3>
          <p>2.1. Сервис принимает разумные организационные и технические меры для защиты данных, но не гарантирует их абсолютную защищенность от взлома или несанкционированного доступа. Передача данных третьим лицам допускается исключительно платежным агрегаторам для проведения транзакций и по законным требованиям правоохранительных органов.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <p>1.1. Возврат денежных средств возможен в течение 24 часов с момента оплаты и только при условии, что услуга не была фактически активирована (предоставлена); при этом возврат осуществляется за вычетом фактических расходов Сервиса, включая комиссии платежных систем. После активации услуги или истечения 24 часов денежные средства не возвращаются.</p>
          <h3 className="font-bold">2. ПОРЯДОК ОБРАЩЕНИЯ</h3>
          <p>2.1. Для оформления возврата Клиент обязан подать обращение в Discord Сервиса с указанием причины, реквизитов и подтверждения оплаты; срок рассмотрения заявки составляет до 10 рабочих дней.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <p>1.1. Сервис гарантирует доступность 99.5% в месяц за исключением периодов Планового и Аварийного технического обслуживания, а также обстоятельств Непреодолимой силы (форс-мажор).</p>
          <h3 className="font-bold">2. ТЕХНИЧЕСКАЯ ПОДДЕРЖКА</h3>
          <p>2.1. Прием обращений в Discord осуществляется 24/7; ориентировочное время ответа — от 30 минут (не является жестким обязательством и зависит от загрузки), при этом поддержка консультирует исключительно по базовой доступности Сервиса и не решает сторонние проблемы Клиента.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-6 text-sm leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <p>1.1. Максимальный совокупный размер ответственности Сервиса saintproject.xyz по любым искам или претензиям ограничен суммой, фактически оплаченной Клиентом за текущий расчетный период (1 месяц) предоставления конкретной Услуги.</p>
          <h3 className="font-bold">2. ИСКЛЮЧЕНИЯ</h3>
          <p>2.1. ервис не несет ответственности за сохранность, целостность или потерю данных Клиента, а также за упущенную выгоду, прошивку/настройки пользовательского ПО, сбои третьих лиц или любые другие косвенные убытки Клиента. Клиент обязан самостоятельно производить резервное копирование (бэкап) своих данных.</p>
        </div>
      </div>
    )
  },
